      ...
      process: Promise.coroutine(function* () {
        try {
          ...
          for (let usergroup of Object.keys(N.config.delete_inactive_users)) {
            let usergroup_id = yield N.models.users.UserGroup.findIdByName(usergroup);
            let period = N.config.delete_inactive_users[usergroup];

            users = users.concat(
              yield N.models.users.User.find()
          ...
          important_usergroups[yield N.models.users.UserGroup.findIdByName('administrators')] = true;
          important_usergroups[yield N.models.users.UserGroup.findIdByName('moderators')] = true;

          for (let user of _.uniqBy(users, u => String(u._id))) {
            if (user.usergroups.some(group => important_usergroups[group])) continue;

            yield N.models.users.User.findById(user._id).remove();
          }
