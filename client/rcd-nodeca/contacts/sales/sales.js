...
N.wire.on('navigate.done:' + module.apiPath, function page_setup() {
  ...
  view.name    = new Control();
  view.email   = new Control();
  view.url     = new Control();
  view.message = new Control();

  view.submitted = ko.observable(false);

  // Only show recaptcha to guests
  if (!N.runtime.is_member) {
...
N.wire.once('navigate.done:' + module.apiPath, function page_once() {
  ...
  N.wire.on(module.apiPath + ':submit', function register(form) {
    ...
    return N.io.rpc('rcd-nodeca.contacts.sales.exec', form.fields)
      .then(() => {
        view.submitted(true);
      })
      .catch(err => {
