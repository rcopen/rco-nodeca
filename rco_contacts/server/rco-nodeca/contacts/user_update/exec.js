...
module.exports = function (N, apiPath) {
  ...
  N.wire.on(apiPath, async function send_email(env) {
    ...
    if (!emails) emails = _.get(N.config, 'rco-nodeca.contacts.user_update.email');
    if (!emails) emails = _.get(N.config, 'rco-nodeca.contacts_default_email');
    if (!emails) emails = [];
    ...
    let text = env.t('email_text', {
      ...
      link:       N.router.linkTo('admin.rco-nodeca.update_by_link', {
